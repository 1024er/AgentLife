...
import floorTextureImage from "/textures/floor.jpg";
...
export const Room = () => {
  ...
  const floorTexture = useMemo(() => {
    const texture = new TextureLoader().load(floorTextureImage);
  ...
  const accumulativeShadows = useMemo(
    () => (
      <AccumulativeShadows
        ...
        alphaTest={0.5}
        scale={map.size[0]}
        position={[map.size[0] / 2, 0, map.size[1] / 2]}
        color="white"
      >
        <RandomizedLight
          amount={4}
  ...
  return (
    <>
      ...
      {(buildMode || shopMode) && (
        <Grid infiniteGrid fadeDistance={50} fadeStrength={5} position-y={-0.1} />
      )}
